...
$MadelineProto = new \danog\MadelineProto\API('BotBot.madeline', $settings);
$MadelineProto->start();
function closeConnection($message = "<br><br><br><center><h1><span style='color:red'>BotBot</span><span style='color:green'>Tabchi</span> <span style='color:gold'>Is</span> <span style='color:purple'>Running</span> !</h1></center>"){
...
try {
if (!file_exists('update-session/BotBot.madeline')){
copy('BotBot.madeline', 'update-session/BotBot.madeline');
}
...
$type2 = $info['type'];
$creator = 208549102; // ایدی عددی ران کننده ربات
$admins = array(1028491954, 208549102); // ایدی عددی ادمین ها

if ($userID != $me_id){
if ($msg == 'تمدید' && $userID == $creator) {
copy('update-session/BotBot.madeline', 'update-session/BotBot.madeline2');
unlink('update-session/BotBot.madeline');
copy('update-session/BotBot.madeline2', 'update-session/BotBot.madeline');
unlink('update-session/BotBot.madeline2');
yield $this->messages->sendMessage(['peer' => $chatID, 'message' => '⚡️ ربات برای 30 روز دیگر شارژ شد']);
}
if (time() - filectime('update-session/BotBot.madeline') > 2505600){
...
yield $this->messages->sendMessage(['peer' => $chatID, 'message' => '🦅 BotBot Tabchi ✅', 'reply_to_msg_id' => $msg_id]);
}
...
$day = (2505600 - (time() - filectime('update-session/BotBot.madeline'))) / 60 / 60 / 24;
$day = round($day, 0);
...
$sessionsize = round(filesize('BotBot.madeline')/1024/1024,2) . 'MB';
yield $this->messages->sendMessage(['peer' => $chatID,
...
if (file_exists('BotBot') and (time()-filectime('BotBot')) > 300) {
unlink('BotBot');
}

if (!file_exists('BotBot')) {
if (file_exists('BotBot.madeline') && filesize('BotBot.madeline')/1024 > 8192){
foreach (glob("*") as $file) {
if ($file != 'index.php' and $file != 'update-session' and $file != 'data.json' and $file != 'index.php') {
@unlink("$file");
}}
copy('update-session/BotBot.madeline', 'BotBot.madeline');
...
yield $this->messages->sendMessage(['peer' => '', 'message' => "$e"]);
}*/
} catch(\danog\MadelineProto\RPCErrorException $p){
// yield $this->messages->sendMessage(['peer' => '', 'message' => "$p"]);
}
}
}
